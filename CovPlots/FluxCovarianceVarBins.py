...
def getHistograms(fileName,color,rebin=1):
    ...
    tmp0   .SetTitle("NuE; True Neutrino Energy [GeV];POT Norm Flux [A.U.] ") 
    tmp1   .SetTitle("NuEBar; True Neutrino Energy [GeV];POT Norm Flux [A.U.] ") 
    tmp2   .SetTitle("NuMu; True Neutrino Energy [GeV];POT Norm Flux [A.U.] ") 
    tmp3   .SetTitle("NuMuBar; True Neutrino Energy [GeV];POT Norm Flux [A.U.] ") 
    #tmpAll .SetTitle("AllNu; True Neutrino Energy [GeV];POT Norm Flux [A.U.] ") 
...
def execute(VariationName = "run18", rebin = 1 ):   
    ...
    if plotCV :
        ...
        for i in xrange(0,4):
            ...
            legend.Draw("same")
            c.Update()
            c.SaveAs("CovPlotsVarBin/"+fhc_histosCV[i].GetTitle()+"_CV.pdf")
            #raw_input()    
    ...
    for i in xrange(0,4):
        ...
        c1.Update()
        #raw_input()
        c1.SaveAs("CovPlotsVarBin/"+tempFHC.GetTitle()+"_"+VariationName+"_Diff.pdf")

    
        NuSpeciesName = diff_FHC[i].GetTitle()
        print NuSpeciesName,  diff_FHC[i].GetSize(),    diff_FHC[i].GetSize()
        f = open("CovPlotsVarBin/"+NuSpeciesName+"_"+VariationName+"_NBins"+str(diff_FHC[i].GetSize()-2)+".csv", "w")
